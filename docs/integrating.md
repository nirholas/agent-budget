# Integrating AgentBudget

## From Solidity

```solidity
import {AgentBudgetHook} from "agent-budget/src/hooks/AgentBudgetHook.sol";
...
const address = hookAddress("agent-budget", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "AgentBudget"
...
More at https://agent-budget.pages.dev.
