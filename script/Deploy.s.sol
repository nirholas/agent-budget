...
/**
 * @title DeployAgentBudget
 * @notice Deploys AgentBudget deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
...
contract DeployAgentBudget is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(AgentBudgetHook).creationCode;
        ...
        console2.log("AgentBudget ", predicted);
        ...
        vm.startBroadcast();
        AgentBudgetHook hook = new AgentBudgetHook{salt: salt}(manager);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("AgentBudget")),
